...
namespace AlphaSoft
{
    public partial class dataSuratTugasForm : Form
    {
        ...
        private void loadInvoiceData()
        {
            ...
            {
                sqlClause1 = "SELECT IF(STH.IS_JOB_FINISHED=1, 'DONE', IF(STH.JOB_SCHEDULED_DATE>=NOW(), 'SCHEDULED', 'PENDING')) AS STATUS, STH.ID, STH.SALES_INVOICE, MC.CUSTOMER_FULL_NAME AS 'CUSTOMER', MT.TECHNICIAN_NAME AS 'NAMA TEKNISI', DATE_FORMAT(STH.SALES_DATE, '%d-%M-%Y')  AS 'TGL INVOICE', DATE_FORMAT(STH.JOB_SCHEDULED_DATE, '%d-%M-%Y')  AS 'TGL TUGAS', DATE_FORMAT(STH.JOB_SCHEDULED_TIME, '%H:%m')  AS 'JAM TUGAS', DATE_FORMAT(STH.JOB_FINISHED_DATE, '%d-%M-%Y')  AS 'TGL SELESAI', DATE_FORMAT(STH.JOB_FINISHED_TIME, '%H:%m')  AS 'JAM SELESAI' " +
