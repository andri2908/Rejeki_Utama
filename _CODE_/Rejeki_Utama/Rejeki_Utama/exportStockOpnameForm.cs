...
namespace AlphaSoft
{
    public partial class exportStockOpnameForm : Form
    {
        ...
        private bool saveToCSV(string fileName)
        {
            ...
            sqlCommand = "SELECT MP.PRODUCT_ID, IFNULL(MP.PRODUCT_BARCODE, ''), IFNULL(MP.PRODUCT_NAME, ''), PL.PRODUCT_LOCATION_QTY, 0 AS PRODUCT_ACTUAL_QTY, '' AS DESCRIPTION FROM MASTER_PRODUCT MP, PRODUCT_LOCATION PL WHERE PL.PRODUCT_ID = MP.PRODUCT_ID AND MP.PRODUCT_ACTIVE = 1 AND PL.LOCATION_ID = " + locationID + " ORDER BY MP.PRODUCT_ID";
