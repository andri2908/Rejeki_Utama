...
namespace AlphaSoft
{
    public partial class dataSalesInvoice : Form
    {
        ...
        private void loadInvoiceData()
        {
            ...
            {
                sqlClause1 = "SELECT REV_NO, ID, SALES_INVOICE AS 'NO INVOICE', CUSTOMER_FULL_NAME AS 'CUSTOMER', DATE_FORMAT(SALES_DATE, '%d-%M-%Y')  AS 'TGL INVOICE', (SALES_TOTAL - ROUND((SALES_TOTAL * SALES_DISC_PERCENT_FINAL) /100) - SALES_DISCOUNT_FINAL) AS 'TOTAL' " +
                                       "FROM SALES_HEADER SH, MASTER_CUSTOMER MC " +
                                       "WHERE SH.CUSTOMER_ID = MC.CUSTOMER_ID AND SH.SALES_VOID = 0 AND SH.SALES_ACTIVE = 1";

                sqlClause2 = "SELECT REV_NO, ID, SALES_INVOICE AS 'NO INVOICE', 'P-UMUM' AS 'CUSTOMER', DATE_FORMAT(SALES_DATE, '%d-%M-%Y') AS 'TGL INVOICE', (SALES_TOTAL - ROUND((SALES_TOTAL * SALES_DISC_PERCENT_FINAL) /100)  - SALES_DISCOUNT_FINAL) AS 'TOTAL' " +
            ...
            {
                sqlClause1 = "SELECT SH.ID, SH.SALES_INVOICE AS 'NO INVOICE', MC.CUSTOMER_FULL_NAME AS 'CUSTOMER', DATE_FORMAT(SH.SALES_DATE, '%d-%M-%Y')  AS 'TGL INVOICE', (SH.SALES_TOTAL - ROUND((SH.SALES_TOTAL * SALES_DISC_PERCENT_FINAL) /100) - SALES_DISCOUNT_FINAL) AS 'TOTAL', IF(STH.SALES_INVOICE IS NOT NULL, 'ST', '') AS 'SURAT TUGAS' " +
                ...
                sqlClause2 = "SELECT SH.ID, SH.SALES_INVOICE AS 'NO INVOICE', '' AS 'CUSTOMER', DATE_FORMAT(SH.SALES_DATE, '%d-%M-%Y') AS 'TGL INVOICE',(SH.SALES_TOTAL - ROUND((SH.SALES_TOTAL * SH.SALES_DISC_PERCENT_FINAL) /100) - SH.SALES_DISCOUNT_FINAL) AS 'TOTAL',  IF(STH.SALES_INVOICE IS NOT NULL, 'ST', '') AS 'SURAT TUGAS' " +
            ...
            if (!showAllCheckBox.Checked)
            {
                if (noInvoiceTextBox.Text.Length > 0)
                {
                    ...
                    else if (originModuleID == globalConstants.SALES_ORDER_REVISION || originModuleID == globalConstants.DELIVERY_ORDER || originModuleID == globalConstants.COPY_DELIVERY_ORDER || originModuleID == globalConstants.TUGAS_PEMASANGAN_BARU)
