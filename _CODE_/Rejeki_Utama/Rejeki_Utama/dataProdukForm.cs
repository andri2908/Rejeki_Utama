...
namespace AlphaSoft
{
    public partial class dataProdukForm : Form
    {
        ...
        private void loadProdukData()
        {
            ...
            {
                sqlCommand = "SELECT MP.ID, MP.PRODUCT_ID, MP.PRODUCT_NAME AS 'NAMA PRODUK', PL.PRODUCT_LOCATION_QTY AS 'QTY' FROM MASTER_PRODUCT MP, PRODUCT_LOCATION PL WHERE PL.PRODUCT_ID = MP.PRODUCT_ID AND PL.LOCATION_ID = " +locationID+ " AND  MP.PRODUCT_ACTIVE = 1 AND MP.PRODUCT_IS_SERVICE = 0 AND MP.PRODUCT_ID LIKE '%" + kodeProductParam + "%' AND MP.PRODUCT_NAME LIKE '%" + namaProductParam + "%'";
            }
