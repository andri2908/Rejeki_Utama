...
namespace AlphaSoft
{
    public partial class suratTugasPrintOutForm : Form
    {
        private globalUtilities gUtil = new globalUtilities();

        public suratTugasPrintOutForm()
        {
            InitializeComponent();
        }

        private void suratTugasPrintOutForm_Load(object sender, EventArgs e)
        {
            DataSet dsTempReport = new DataSet();
            try
            {
                string appPath = Directory.GetCurrentDirectory() + "\\" + globalConstants.suratTugasXML;
                dsTempReport.ReadXml(@appPath);

                //prepare report for preview
                suratTugas rptXMLReport = new suratTugas();
